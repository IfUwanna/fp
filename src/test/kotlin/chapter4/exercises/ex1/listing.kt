...
// 4.1 Option 데이터 타입 개선하기
// Option이 None이 아닌 경우 f를 적용해 A타입 값을 B타입으로 반환
...
// Option이 None이 아닌 경우 실패할 수도 있는 f를 적용해 A타입값을 B타입값으로 변환
...
// 술어 f를 만족하지 않으면 Some > None 변환하여 반환
fun <A> Option<A>.filter(f: (A) -> Boolean): Option<A> = when (this) {
    is None -> None
    is Some -> if (!f(this.get)) {
        None
    } else this
}
